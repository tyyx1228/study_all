...
object IPLocation {
  ...
  val data2MySQL = (iterator: Iterator[(String, Int)]) => {
    ...
    try {
      conn = DriverManager.getConnection("jdbc:mysql://hdp10:3306/bigdata?useUnicode=true&characterEncoding=UTF-8", "root", "root")
  ...
  def main(args: Array[String]) {
    ...
    val ipRulesRdd = sc.textFile("E:\\wordcount\\spark\\iplistforchina\\ip.txt").map(line =>{
    ...
    //加载要处理的数据
    val ipsRDD = sc.textFile("E:\\wordcount\\spark\\accesslog\\access.log").map(line => {
      val fields = line.split("\\|")
      fields(1) //从访问日志中提取ip字段
    })

    val result = ipsRDD.map(ip => {
      val ipNum = ip2Long(ip) //ip转为long类型
