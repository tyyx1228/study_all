...
from (
	--只同步2018年7月1号及以后的报警事件
	--注: oracle中从tes_ba_t_p_w_list中删除报警事件数据时，如果该事件受控，则不能删除
	--此处tes_ba_t_p_w_list表可能较大，而baojing_info_sc表远小于tes_ba_t_p_w_list，这里不采用join方式而采用子查询方式
	select * from tes_ba_t_p_w_list
	where w_id in (
	  select id from baojing_info_sc where last_update_date>=:sql_last_value
	)
) t
